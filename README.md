...
- The `models` folder contains the stored model as a `pickle` file.
- The `web-service` folder contains the code for deploying the trained model as a web service with `Docker` and `Flask`.
- The `monitoring` folder contains code for monitoring the model with `Evidently` and `Grafana`.
...
- **Makefile**
  - Defines the `make` instructions.
- **.pre-commit-config.yaml**
  - Defines the configuration for the Git pre-commit hooks.
- **ml_pipeline-deployment.yaml**
  - The autogenerated configuration file from the `Prefect` workflow orchestrator.
- **pyproject.toml**
  - The configuration file for formatting (`black` and `isort`) and linting packages (`pylint`).
