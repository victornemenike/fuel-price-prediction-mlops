...
column_mapping = ColumnMapping(prediction='e5_forecasted', target=None)
...
def calculate_metrics_postgresql(curr, i):
    ...
    _, _, forecasted_values, combined_index, sequence_to_plot = forecast(
        loaded_model, data_name, current_data, num_forecast_steps
    )
    original_cases = np.expand_dims(sequence_to_plot[-1], axis=0).flatten()
    last_npoints = 24
    monitored_data = pd.DataFrame(
        {
            'timestamp': combined_index[-last_npoints:],
            'e5': original_cases,
    ...
    curr.execute(
        ...
        (
            begin + datetime.timedelta(hours=i),
            prediction_drift,
            num_drifted_columns,
